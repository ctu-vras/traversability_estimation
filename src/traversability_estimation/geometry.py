...
class Body(object):
    ...
    def distance(self, x):
        ...
        if x.ndim == 1:
            x = x.reshape((3, -1))
        return self.distance_local(affine(self.pose_inv, x))
...
class Bodies(Body):
    ...
    >>> Bodies([Sphere(0.5), Sphere(0.5, origin=[1, 0, 0])]).distance([[0], [0], [0]]).item()
    -0.5
    >>> Bodies([Sphere(0.5), Sphere(1.0, origin=[1, 0, 0])]).distance([[1], [0], [0]]).item()
    -1.0
    >>> Bodies([Sphere(0.5), Box(origin=[1, 0, 0])]).distance([[0.5], [0.5], [0.5]]).item()
    0.0
    """
    ...
    def distance_local(self, x):
        ...
        for body in self.bodies:
            d = np.minimum(d, body.distance(x))
        return d

    def contains_local(self, x):
        c = np.zeros((x.shape[1],), dtype=bool)
        for body in self.bodies:
            c = np.logical_or(c, body.contains(x))
        return c
...
class Box(Body):
    ...
    >>> Box().contains([0, 0, 0]).item()
    True
    >>> Box().contains([0, 0, -0.5]).item()
    True
    >>> Box().contains([0, 0, -1.0]).item()
    False
    >>> Box().distance([0, 0, 0]).item()
    -0.5
    >>> Box().distance([3.5, 4.5, 0]).item()
    5.0
    """
...
class Sphere(Body):
    ...
    >>> Sphere().contains([0, 0, 0]).item()
    True
    >>> Sphere().contains([0, 0, -1]).item()
    True
    >>> Sphere().contains([0, 0, -2]).item()
    False
    >>> Sphere().distance([0, 0, 0]).item()
    -1.0
    >>> Sphere().distance([0, 1, 0]).item()
    0.0
    >>> Sphere().distance([0, 2, 0]).item()
    1.0
    """
...
class Cylinder(Body):
    ...
    >>> Cylinder(height=2.0).contains([0, 0, 1.0]).item()
    True
    >>> Cylinder(height=2.0).contains([0, 0, -2.0]).item()
    False
    >>> Cylinder().distance([0, 0, 0]).item()
    -0.5
    >>> Cylinder().distance([0, 0, -1.0]).item()
    0.5
    >>> Cylinder().distance([0, 2, 0]).item()
    1.0
    >>> Cylinder().distance([4, 0, 4.5]).item()
    5.0
    """
